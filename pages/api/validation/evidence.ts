// API Route: /api/validation/evidence
// Purpose: Run agent validation layer on evidence items
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const evidenceId = params.evidence_id ? String(params.evidence_id) : undefined;
  const firmId = params.firm_id ? String(params.firm_id) : undefined;
  const limit = params.limit ? Math.min(Number(params.limit) || 20, 200) : 20;

  if (!evidenceId && !firmId) {
    return res.status(400).json({ error: 'evidence_id or firm_id required' });
  }
  ...
  try {
    const rows = evidenceId
      ? await fetchEvidenceById(dbPool, evidenceId)
      : await fetchEvidenceByFirm(dbPool, firmId as string, limit);
