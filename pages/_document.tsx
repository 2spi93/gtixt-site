      ...
      <Html lang="en">
        <Head>
          ...
          <script
            dangerouslySetInnerHTML={{
              __html:
                "(function(){if(typeof window==='undefined')return;var KEY='__chunk_reload_count__';var MAX=3;function isChunkErr(v){if(!v)return false;var m=(v&&v.message)||String(v||'');return m.indexOf('ChunkLoadError')!==-1||m.indexOf('Loading chunk')!==-1||m.indexOf('Failed to load chunk')!==-1||m.indexOf('/_next/static/chunks/')!==-1;}function tryRecover(){try{var attempts=parseInt(sessionStorage.getItem(KEY)||'0',10)||0;if(attempts>=MAX)return;attempts+=1;sessionStorage.setItem(KEY,String(attempts));var url=new URL(window.location.href);url.searchParams.set('__chunk_retry',String(attempts));url.searchParams.set('__chunk_ts',String(Date.now()));window.location.replace(url.toString());}catch(_){window.location.reload();}}window.addEventListener('error',function(e){if(isChunkErr(e&&e.error)||isChunkErr(e&&e.message)){tryRecover();}},true);window.addEventListener('unhandledrejection',function(e){if(isChunkErr(e&&e.reason)){tryRecover();}},true);})();",
            }}
          />
