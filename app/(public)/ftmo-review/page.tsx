...
export default async function FtmoReviewPage() {
  ...
  const prediction = ftmo ? buildRiskPrediction(ftmo) : null
  ...
  const predictionHistory = ftmo ? await queryPredictionHistory(ftmo.firm_id || 'ftmo', 8) : []
  const advancedPrediction = ftmo
    ? buildAdvancedRiskPrediction(ftmo, firms.filter((f) => f.firm_id !== ftmo.firm_id), systemicRisk.stressRatio, predictionHistory)
    : null
      ...
      <div className="max-w-4xl mx-auto space-y-10">
        ...
        {advancedPrediction && riskTrendData.length >= 2 && (
                    <section className="space-y-4">
                      ...
                      <RiskTrendChart firmName="FTMO" data={riskTrendData} trend={advancedPrediction.trend} />
                    </section>
        )}
